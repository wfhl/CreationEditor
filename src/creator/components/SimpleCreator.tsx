...
import { Edit2, Play, PenTool, Folder } from 'lucide-react';
...
import type { DBAsset as Asset, DBPromptPreset, DBGenerationHistory } from '../lib/dbService';
...
import { useAuth } from './AuthProvider'; // stub — always returns null user

interface SimpleCreatorProps {
    /** Which creator tab to show — driven by the ClipVid sidebar */
    externalTab?: 'create' | 'edit' | 'scripts' | 'animate' | 'refine' | 'script' | 'settings' | 'assets';
    /** Called when the user clicks "Send to Editor" — receives all generated URLs */
    onSendToEditor?: (urls: string[]) => void;
    /** Navigate to a different sidebar tab (e.g. 'media' to switch to the editor) */
    onNavigateTo?: (tab: string) => void;
}

export default function SimpleCreator({ externalTab, onSendToEditor, onNavigateTo }: SimpleCreatorProps) {
    useAuth(); // Keep stub imported to avoid TS errors if referenced downstream
    // --- Shared State ---
    const [assets, setAssets] = useState<Asset[]>([]);
    const initialTab = (() => {
        if (externalTab === 'refine') return 'edit';
        if (externalTab === 'script') return 'scripts';
        return externalTab || 'create';
    })();
    const [activeTab, setActiveTab] = useState<'create' | 'assets' | 'edit' | 'animate' | 'scripts' | 'settings'>(initialTab as any);
    ...
    // Sync external tab (driven by ClipVid sidebar) into internal activeTab
    useEffect(() => {
        if (externalTab) {
            let mappedTab = externalTab as string;
            if (externalTab === 'refine') mappedTab = 'edit';
            if (externalTab === 'script') mappedTab = 'scripts';
            setActiveTab(mappedTab as any);
        }
    }, [externalTab]);

    // Handler: send all current media to ClipVid media library and navigate there
    const handleSendToEditor = () => {
        const urls = [
            ...generatedMediaUrls,
            ...(i2vResultUrl ? [i2vResultUrl] : []),
            ...refineResultUrls,
        ].filter(Boolean);
        onSendToEditor?.(urls);
        onNavigateTo?.('media');
    ...
    useEffect(() => {
        const loadInitialLocalData = async () => {
            try {
                const savedThemes = await dbService.getConfig('themes');
                if (savedThemes) setThemes(savedThemes);
                else {
                    setThemes(SIMPLE_THEMES as Theme[]);
                    await dbService.saveConfig('themes', SIMPLE_THEMES);
                }

                const savedStyles = await dbService.getConfig('caption_styles');
                if (savedStyles) setCaptionStyles(savedStyles);
                else {
                    setCaptionStyles(CAPTION_TEMPLATES as CaptionStyle[]);
                    await dbService.saveConfig('caption_styles', CAPTION_TEMPLATES);
                }

                const savedProfile = await dbService.getConfig('creator_profile');
                if (savedProfile) setProfile(savedProfile);
                else {
                    setProfile(SIMPLE_PROFILE);
                    await dbService.saveConfig('creator_profile', SIMPLE_PROFILE);
                }
...
TECHNICAL PROMPT: ${finalPromptToUse}`;
    ...
    const handleSavePost = async () => {
        if (generatedMediaUrls.length === 0) return;
        setIsSavingPost(true);
        try {
            // Primarily send to the main project editor
            if (onSendToEditor) {
                onSendToEditor(generatedMediaUrls);
                alert("Assets sent to Your Media!");
            }
            
            // Optionally still save a background entry in DB (no UI for it now though)
            /*
    ...
    return (
        <div className="h-full w-full bg-[var(--bg-deep)] text-white font-sans selection:bg-purple-500/30 selection:text-purple-100 flex flex-col overflow-hidden">
            {/* Header removed as per USER request - Navigation is now handled by the SidebarRail */}
            ...
            {/* Mobile Bottom Navigation removed — now handled by unified sidebar */}
                        ...
                        <div className="mt-6 shrink-0 flex flex-wrap justify-center gap-3 md:gap-4 pb-4 px-4 w-full">
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    onSendToEditor?.(previewUrl, isVideo ? 'video' : 'image');
                                }}
                            ...
                            >
                                <Folder className="w-3 md:w-4 h-3 md:h-4" />
                                Send to Media
                            </button>
                        </div>
                    </div>
